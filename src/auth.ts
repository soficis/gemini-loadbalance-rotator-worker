...
import {
	...
	OAUTH_REFRESH_URL,
	TOKEN_BUFFER_TIME,
	getKvTokenKey // IMPORT a new function
} from "./config";
...
export class AuthManager {
	...
	private credential: OAuth2Credentials; // ADDED: store the specific credential
	private kvTokenKey: string; // ADDED: store the specific KV key

	constructor(env: Env, credentialJson: string, index: number) {
		this.env = env;
		if (!credentialJson) {
			throw new Error(`Credential for index ${index} is not provided.`);
		}
		this.credential = JSON.parse(credentialJson);
		this.kvTokenKey = getKvTokenKey(index);
	}
	...
	public async initializeAuth(): Promise<void> {
		try {
			...
			try {
				// USE the dynamic KV key
				const cachedToken = await this.env.GEMINI_CLI_LOADBALANCE.get(this.kvTokenKey, "json");
				if (cachedToken) {
					cachedTokenData = cachedToken as CachedTokenData;
					console.log(`Found cached token in KV storage for key ${this.kvTokenKey}`);
				}
			} catch (kvError) {
				console.log(`No cached token found for key ${this.kvTokenKey} or KV error:`, kvError);
			}
			...
			// USE this.credential directly
			const oauth2Creds: OAuth2Credentials = this.credential;
			...
			console.log(`All tokens for key ${this.kvTokenKey} expired, refreshing...`);
	...
	private async cacheTokenInKV(accessToken: string, expiryDate: number): Promise<void> {
		try {
			...
			if (ttlSeconds > 0) {
				// USE the dynamic KV key
				await this.env.GEMINI_CLI_LOADBALANCE.put(this.kvTokenKey, JSON.stringify(tokenData), {
					expirationTtl: ttlSeconds
				});
				console.log(`Token cached in KV storage for key ${this.kvTokenKey} with TTL of ${ttlSeconds} seconds`);
			} else {
	...
	public async clearTokenCache(): Promise<void> {
		try {
			// USE the dynamic KV key
			await this.env.GEMINI_CLI_LOADBALANCE.delete(this.kvTokenKey);
			console.log(`Cleared cached token from KV storage for key ${this.kvTokenKey}`);
		} catch (kvError) {
			console.log(`Error clearing KV cache for key ${this.kvTokenKey}:`, kvError);
		}
	}
